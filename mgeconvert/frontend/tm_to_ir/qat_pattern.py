...
def gen_qat_conv_opr(
    module, conv_function_expr, qat_expr, irgraph, resolver, is_deconv=False
):
    ...
    if len(conv_function_expr.inputs) == 3:
        ...
        assert (isinstance(bias.expr, GetAttr) and bias.expr.name == "bias") or (
            ...
            and isinstance(bias.expr.opdef, builtin.FakeQuant)
        )
    assert (
...
def qat_conv_bias_relu(module, expr, call_expr, irgraph, resolver):
    relu = expr.inputs[1].expr
    op = gen_qat_conv_opr(module, relu.inputs[0].expr, call_expr, irgraph, resolver)
    op.activation = "RELU"
    return op


@register_fusion_pattern(pat_conv_bias)
def qat_conv_bias(module, expr, call_expr, irgraph, resolver):
    conv = expr.inputs[1].expr
    op = gen_qat_conv_opr(module, conv, call_expr, irgraph, resolver)
    return op


@register_fusion_pattern(pat_conv_relu)
def qat_conv_relu(module, expr, call_expr, net, resolver):
    relu = expr.inputs[1].expr
    op = gen_qat_conv_opr(module, relu.inputs[0].expr, call_expr, net, resolver)
    op.activation = "RELU"
    return op


@register_fusion_pattern(pat_conv)
def qat_conv(module, expr, call_expr, net, resolver):
    conv = expr.inputs[1].expr
    op = gen_qat_conv_opr(module, conv, call_expr, net, resolver)
    return op
...
def qat_deconv_relu_bias(
    ...
    op = gen_qat_conv_opr(module, deconv, call_expr, irgraph, resolver, is_deconv=True)
    op.activation = "RELU"
    ...
    relu_op.out_tensors.append(resolver.resolve(call_expr.outputs[0], relu_op))
