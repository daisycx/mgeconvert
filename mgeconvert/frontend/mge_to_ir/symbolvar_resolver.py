from mgeconvert.converter_ir.ir_graph import IRGraph
from mgeconvert.converter_ir.ir_tensor import AxisOrder, IRTensor
...
class SymbolVarResolver:
    ...
    def resolve(
        self, sym_var, owner_opr=None, param_name=None, axis_order=AxisOrder.NCHW
    ):
        name = sym_var.name
        name = name.replace(":", "_")
        name = name.replace(".", "_")
    ...
    def get_ir_tensor(
        ...
        user_opr=None,
        name=None,
        axis_order=AxisOrder.NCHW,
    ):
        ir_tensor = self.resolve(
            sym_var, owner_opr=owner_opr, param_name=name, axis_order=axis_order
        )
